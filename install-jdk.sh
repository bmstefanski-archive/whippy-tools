...
function determine_url() {
    ...
    local candidates=$(wget --quiet --output-document - ${JAVA_NET} | grep -Eo 
'href[[:space:]]*=[[:space:]]*"[^\"]+"' | grep -Eo '(http|https)://[^"]+')
    if [[ "${feature}" == "${latest_jdk}" ]]; then
        url=$(echo "${candidates}" | grep -Eo 
"${DOWNLOAD}/.+/jdk${feature}/.+/${license}/.*jdk-${feature}.+linux-x64_bin.tar.gz$")
        return
    fi
